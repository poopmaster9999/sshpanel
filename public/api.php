<?php
/**
 * SSH Panel Manager - API Endpoints
 */
...
// Sync online status on each request
try {
    syncOnlineStatus();
} catch (Exception $e) {
    // Silently continue if sync fails (non-root mode)
}
...
switch ($action) {
    ...
    case 'admin_login':
        ...
        // Fallback for default password (not hashed on first install)
        if ($username === 'admin' && $password === 'admin' && $adminPass === password_hash('admin', PASSWORD_DEFAULT)) {
            $_SESSION['admin_logged_in'] = true;
            $_SESSION['admin_username'] = $username;
            jsonResponse(['success' => true]);
        }
        // Direct compare fallback
        if ($username === $adminUser && ($password === $adminPass || password_verify($password, $adminPass))) {
            $_SESSION['admin_logged_in'] = true;
            $_SESSION['admin_username'] = $username;
            jsonResponse(['success' => true]);
        }
    ...
    case 'get_user':
        $id = (int)($_GET['id'] ?? 0);
        $user = getUserById($id);
        if (!$user) jsonResponse(['success' => false, 'error' => 'Not found'], 404);
        jsonResponse(['success' => true, 'user' => $user]);
        break;
    ...
    case 'add_chat_id':
        ...
        if (empty($chatId)) jsonResponse(['success' => false, 'error' => 'Empty chat ID'], 400);
    ...
    case 'get_stats':
        ...
        $stats['os'] = trim(shell_exec("lsb_release -ds 2>/dev/null") ?? 'Ubuntu 22.04');
